interface Set<T> {
    ...
    every<S extends T>(callback: (value: T) => value is S, thisArg?: any): this is Set<S>;
    every(callback: (value: T) => unknown, thisArg?: any): boolean;

    filter<S extends T>(callback: (value: T) => value is S, thisArg?: any): Set<S>;
    filter(callback: (value: T) => unknown, thisArg?: any): Set<T>;

    find<S extends T>(callback: (value: T) => value is S, thisArg?: any): S | undefined;
...
interface MapConstructor {
    ...
    keyBy<T, K>(callback: (value: T) => K): Map<string, T>;
}
...
interface Map<K, V> {
    ...
    findKey(callback: (value: V, key: K) => boolean, thisArg?: any): K | undefined;

    includes(searchElement: V): boolean;
    keyOf(searchElement: V): K | undefined;

    mapKeys<K1>(mapFn: (value: K, index: number) => K1, thisArg?: any): Map<K1, V>;
    mapValues<V1>(mapFn: (value: V, index: number) => V1, thisArg?: any): Map<K, V1>;
    ...
    update(key: K, callback: (value: V, key: K) => V, thunk?: (key: K) => K): this;
}
